...
load_ici_engine_functions <- function() {
  if (isTRUE(get0("ici_engine_loaded", ifnotfound = FALSE))) {
...
export_all_subsets_analysis <- function(result_dir) {
  ...
  if (n_ok < 1) {
    log_msg("all-subsets: no panel embeddings under ", result_dir, " ; run ICI_Flow_dimred_pipeline.R first")
