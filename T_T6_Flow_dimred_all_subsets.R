#!/usr/bin/env Rscript
# =============================================================================
# T-T6：P1/P2/P3 亚群频率总览（T6 vs T）
# 只汇总本方案 results_flow/，不读 E:/R/fuction of cell 或 Internation。
# 入口仍是 source("T_T6_Flow_dimred_pipeline.R")
# =============================================================================
...
export_all_subsets_analysis <- function(result_dir) {
  ...
  if (n_ok < 1) {
    log_msg("all-subsets: no panel embeddings under ", result_dir, " ; run T_T6_Flow_dimred_pipeline.R first")
  ...
  note <- c(
    ...
    "Stats: keep all 3 bio-reps per group (n=3); do not drop an extreme.",
  ...
  save_gg(plot_all_freq_facet(freq_plot, "All subsets (within-panel %)  T6 vs T"),
          file.path(out_dir, "all_subsets_frequency_T6_vs_T"),
          width = w_facet, height = 5.8)
  save_gg(plot_all_stacked(freq_plot, "Within-panel composition (mean of n=3 bio-reps)"),
