#!/usr/bin/env Rscript
# =============================================================================
# JZ：免疫细胞亚群降维（比较 JZ-AB / JZ-EVB；P1 T/NK，P2 B，P3 髓系）
#
# 独立于 E:/R/fuction of cell 的 Flow_dimred_pipeline.R，也独立于 ICI_* 和 JY_*。
# 圈门、联合降维、技术重复平均后再去掉 1 个极端生物学重复（n=2）与原免疫亚群方案一致。
# 染色仍是原 P1/P2/P3（JZ_flow_panel_map.json），不是 His 靶细胞表。
#
# 组别：JZ-EVB（EVB-1/2/3 × 技术重复 EVB1-1/EVB1-2）
#       JZ-AB（AB-1/2/3 × 技术重复 AB1-1/AB1-2）
#
...
jz_script_dir <- jz_get_script_dir()
...
# 只加载本目录的 JZ_flow_engine.R，禁止去 fuction of cell / Internation 找原流程
load_jz_engine <- function() {
  pipe <- "JZ_flow_engine.R"
  cands <- unique(c(
    file.path(jz_script_dir, pipe),
  ...
  if (is.na(hit) || !nzchar(hit)) {
    stop(
      "找不到 JZ_flow_engine.R。JZ 方案与免疫亚群 / 靶细胞方案完全独立，",
      "不会去 E:/R/fuction of cell 找 Flow_dimred_pipeline.R，也不会用 ICI_* 或 JY_*。\n",
      "请把这些文件一起放到 E:/R/fuction of cell-wjz：\n",
  ...
  if (exists("log_msg", mode = "function")) log_msg("JZ engine (this scheme only): ", hit)
  invisible(TRUE)
}
...
find_jz_panel_map <- function() {
  names <- c("JZ_flow_panel_map.json", "JZ_flow_panel_map.json.txt")
  dirs <- unique(c(
    getwd(),
    jz_script_dir,
    jz_primary_data_dir,
  ...
  dirs <- dirs[vapply(dirs, jz_keep_cand, logical(1))]
  for (d in dirs) {
  ...
  stop("找不到 JZ_flow_panel_map.json（应与 JZ_Flow_dimred_pipeline.R 同目录，或放在 E:/R/fuction of cell-wjz）")
}

resolve_jz_dir <- function() {
  ...
  preferred <- preferred[vapply(preferred, jz_keep_cand, logical(1))]
  for (d in preferred) {
...
panel_map <- read_panel_map_file(find_jz_panel_map())
log_msg("JZ panel map: ", find_jz_panel_map())
...
flow_primary_data_dir <- jz_primary_data_dir
project_dir <- resolve_jz_dir()
result_dir <- file.path(project_dir, "results_flow")
log_dir <- file.path(result_dir, "00_logs")
dir.create(log_dir, recursive = TRUE, showWarnings = FALSE)

find_jz_extra <- function(nm) {
  cands <- unique(c(
    file.path(jz_script_dir, nm),
    file.path(getwd(), nm)
  ))
  cands <- cands[vapply(cands, jz_keep_cand, logical(1))]
  hit <- cands[file.exists(cands)][1]
...
if (identical(toupper(Sys.getenv("FLOW_FUNCTIONS_ONLY", "0")), "1") ||
  ...
  log_msg("JZ FLOW_FUNCTIONS_ONLY=1: skip analysis")
} else {

log_msg("JZ flow dir: ", project_dir)
log_msg("JZ results: ", result_dir)
log_msg("JZ purpose: immune-subset dimred, comparison JZ-AB / JZ-EVB (same gates as original P1/P2/P3)")
log_msg("JZ stats: average tech reps, then drop 1 extreme bio-rep (max or min) per group; n=2")
...
if (nrow(file_tab) == 0) {
  if (demo_flag) {
    ...
    log_msg("Put JZ-EVB / JZ-AB unmixed files (EVB1-1, AB1-2, …) in ", jz_primary_data_dir,
            ", or set FLOW_DEMO=1")
    use_demo <- TRUE
  ...
  for (pn in c("P1", "P2", "P3")) {
    ...
    if (n_pn == 0) {
      log_msg(pn, " missing. Put files named like JZ-EVB1-1_", pn, "_unmixed.fcs or EVB1-1_", pn, "_unmixed.fcs")
    }
  }
  if (any(file_tab$group == "JZ-AB" & grepl("EVB", file_tab$file, ignore.case = TRUE))) {
    stop("Filename parser classified an EVB file as JZ-AB; refusing to continue")
  }
  ensure_flowcore()
}
...
sum_path <- file.path(result_dir, "JZ_AB_vs_JZ_EVB_lineage_stats_all_panels.csv")
...
extra <- find_jz_extra("JZ_Flow_dimred_all_subsets.R")
...
traj <- find_jz_extra("JZ_Flow_dimred_trajectory.R")
...
log_msg("JZ done. Keep JZ_* files in ", jz_primary_data_dir,
