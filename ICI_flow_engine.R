...
# 读写目录：只在 Internation cell immune；不要回退到 fuction of cell
flow_primary_data_dir <- "E:/R/Internation cell immune"
flow_legacy_data_dir <- "E:/R/Internation cell immune"
flow_legacy_data_dir2 <- "E:/R/Internation cell immune"
...
# -----------------------------------------------------------------------------
# 2. 文件名：ZZX-EV / ZZX-H（本方案：EV-1 / H-2，通常无技术重复）
#    入口脚本 ICI_Flow_dimred_pipeline.R 会再覆盖 parse_fcs_filename。
#    先匹配 EV，避免被单字母 H 误伤
# -----------------------------------------------------------------------------
flow_ctrl_group <- "EV"
flow_trt_group <- "H"
...
# ICI 靶细胞实验 n=3，不去极端生物学重复（与免疫亚群方案相反）。
...
find_panel_map_file <- function(dirs = panel_map_search_dirs()) {
  ...
  fuzzy <- unlist(lapply(dirs, function(d) {
    list.files(d, pattern = "^ICI_flow_panel_map", ignore.case = TRUE, full.names = TRUE)
  }), use.names = FALSE)
...
load_panel_map <- function() {
  ...
  stop(
    "找不到 ICI_flow_panel_map.json。靶细胞方案不读 fuction of cell 里的 flow_panel_map.json。\n",
    "Windows 若类型是“文本文档”，真实文件名多半是 ICI_flow_panel_map.json.txt。\n",
    "已搜索：\n", listing
  )
}
...
# -----------------------------------------------------------------------------
# 功能状态（图1 布局）：只在 His-FITC+ 母群上做，亚群与免疫降维同一套
# -----------------------------------------------------------------------------
...
ici_keep_his_parent_cells <- function(cells) {
  if (is.null(cells) || !nrow(cells)) return(cells)
  if (!"His" %in% names(cells)) {
    log_msg("functional-state: no His column; embeddings should already be the His+ parent")
    return(cells)
  }
  his <- as.numeric(cells$His)
  if (!any(is.finite(his))) return(cells)
  cut <- axis_pos_cut(his)
  keep <- is.finite(his) & is.finite(cut) & his >= cut
  keep[is.na(keep)] <- FALSE
  if (!any(keep) && is.finite(stats::median(his, na.rm = TRUE)) &&
      stats::median(his, na.rm = TRUE) >= 1.2) {
    return(cells)
  }
  n0 <- nrow(cells)
  out <- cells[keep, , drop = FALSE]
  if (nrow(out) < n0) {
    log_msg("functional-state: keep His-FITC+ parent ", nrow(out), "/", n0)
  }
  out
}
...
export_one_functional_state <- function(cells, panel_id, spec, out_dir) {
  ...
  label <- paste0(panel_id, " His+ ", parent, " ", spec$state)
  ...
  log_msg(panel_id, " His+ functional-state ", parent, " ", spec$state, " -> ", stub)
  invisible(stub)
}

export_functional_state_figures <- function(cells, panel_id, out_dir) {
  cells <- ici_keep_his_parent_cells(cells)
  if (is.null(cells) || nrow(cells) < 20) {
    log_msg(panel_id, " skip functional-state (need His+ cells)")
    return(invisible(NULL))
  }
  ...
  log_msg(panel_id, " functional-state on His-FITC+ parent only")
  if (identical(panel_id, "P1")) {
    log_msg("P1 His+: activation/effector listed as CD69/IFN-g/TNF-a/GZMB; this sheet has CD69 (skip missing). Exhaustion PD-L1/LAG-3/TIM-3 skip if absent. NKG2D is MFI only.")
  }
  if (identical(panel_id, "P2")) {
    log_msg("P2 His+ B: no CD86/CD80/CD40 on this 7-color sheet; skip activation item, do not invent exhaustion")
  }
  if (identical(panel_id, "P3")) {
    log_msg("P3 His+ myeloid: activation CD86/CD80/CD40 (TNF-a/IL-6 skip if absent); suppression IL-10/TGF-b skip if absent; not exhaustion; skip dump T/B/NK")
  }
  for (spec in specs) {
    tryCatch(
  ...
  log_msg(panel_id, " His+ functional-state figures: ", func_dir)
  invisible(TRUE)
}
...
export_functional_state_from_results <- function(result_dir) {
  ...
  if (!dir.exists(result_dir)) {
    stop("找不到 results_flow：", result_dir, "。请 setwd 到 Internation cell immune 且已经出过总结果。")
  }
  log_msg("Standalone ICI functional-state: His+ parent only; no FCS, no UMAP; read embeddings in ", result_dir)
  log_msg(
    "ALL gated His+ subsets: P1 specs=", length(functional_state_specs("P1")),
    " P2 specs=", length(functional_state_specs("P2")),
    " P3 specs=", length(functional_state_specs("P3")),
    " (missing stain channels skip that item)"
  )
  if (!isFALSE(flow_trim_bio_extremes)) {
    log_msg("ICI functional-state keeps n=3; do not drop extreme bio-reps")
  }
  n_ok <- 0L
  ...
  if (!n_ok) {
    stop(
      "没有可读的 P1/P2/P3_cell_embeddings.csv。先跑完 ICI_Flow_dimred_pipeline.R，",
      "再单独 source ICI_Flow_dimred_functional_state.R。"
    )
  }
  log_msg("ICI His+ functional-state rerun done. Figures: results_flow/P1|P2|P3/functional_state/")
  invisible(TRUE)
}

ici_engine_loaded <- TRUE

# -----------------------------------------------------------------------------
# 10. 本文件只提供函数。分析入口是 ICI_Flow_dimred_pipeline.R
# -----------------------------------------------------------------------------
log_msg("ICI_flow_engine.R: function library only; run ICI_Flow_dimred_pipeline.R")
