...
# 近/远沿用主脚本：近 ≤2 spot，远 >4 spot。
# FC>1.25 上调基因：每个病人各画一张高低表达 vs 施旺距离，不画把病人拼在一起的总图。
...
# 本脚本只要上调 FC>1、1.25；距离图：每个 FC>1.25 基因 × 每个病人各一张
p_cutoff <- 0.05
fc_cutoffs_up <- c("FC_gt_1" = 1, "FC_1.25" = 1.25)
# Wang ST spot 中心距约 100 μm；距离用 spot 网格换算成 mm
spot_pitch_mm <- 0.1
# 近/远沿用 classify_one_patient 默认：近 ≤2 spot，远 >4 spot
# 距离参照：施旺细胞签名
...
# 上一版 2/10 spot 留下的说明，避免和现在的 2/4 混在一起
stale_cutoffs <- c(
  file.path(out_root, "DISTANCE_CUTOFFS.txt"),
  file.path(out_root, "DISTANCE_per_patient.csv")
)
unlink(stale_cutoffs[file.exists(stale_cutoffs)])
...
plot_high_low_vs_distance <- function(df, outfile, gene_lab, xlab,
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = dist_mm, colour = grp)) +
    ...
    ggplot2::labs(
      title = if (is.null(plot_title) || !nzchar(plot_title)) NULL else plot_title,
      x = xlab,
      y = "Density"
    )
...
writeLines(c(
  ...
  "近/远：近 ≤2 spot，远 >4 spot。1 spot ≈ 0.1 mm。",
  "",
  ...
  "  不要打开把所有病人拼在一起的总图（重跑会删掉）",
  "  本次要画的上调基因.txt",
  "  INDEX_FC_1.25_each_gene_each_patient.csv",
  "  FC_1.25_each_gene/基因名/TNBC病人_该基因_高低表达_vs_施旺距离.pdf",
...
log_msg("Near/far cutoffs: near<=2 spot, far>4 spot (1 spot ~ 0.1 mm)")
...
for (pid in selected_patients) {
  ...
  if (!elig) {
    row$note <- paste0("近神经肿瘤 spot=", length(n_near_s),
                       ...
                       "（需要近>=5 且 远>=10；近≤2 spot，远>4 spot）")
    status_rows[[as.character(pid)]] <- row
    rm(obj, cnts, logm)
    gc(verbose = FALSE)
    next
...
plot_each_fc125_gene <- function(up_tab) {
  ...
  stale_idx <- file.path(dist_dir, "INDEX_FC_1.25_each_gene.csv")
  if (file.exists(stale_idx)) unlink(stale_idx)
  dir.create(gene_dir, recursive = TRUE, showWarnings = FALSE)

  writeLines(c(
    "只画「远神经肿瘤 vs 近神经肿瘤、FC>1.25 上调」的那些基因。",
    "每个基因、每个病人各一张图：该病人该基因的高/低表达 vs 到施旺的距离。",
    "不要看把所有病人拼在一起的总图，也不要看旧的 genes/（ARTN、NGF 不是这批上调基因）。",
    "名单：INDEX_FC_1.25_each_gene_each_patient.csv 和 本次要画的上调基因.txt",
    "施旺 marker：SOX10, MPZ, PMP22, S100B, PLP1, NGFR, NCAM1, MBP, L1CAM。",
    "红 = 该病人该基因高表达，蓝 = 该病人该基因低表达。1 spot = 0.1 mm。",
    "近/远：近 ≤2 spot，远 >4 spot。",
    "这是空间转录组 RNA，不是蛋白质组。"
  ), file.path(dist_dir, "00_READ_ME.txt"))

  if (is.null(up_tab) || nrow(up_tab) == 0 || length(sig_cache) == 0) {
    log_msg("没有 FC>1.25 上调基因，或没有肿瘤 spot，跳过逐病人距离图")
  ...
  log_msg("FC>1.25 上调基因共 ", length(genes), " 个，每个病人各画一张：")
  log_msg(paste(genes, collapse = ", "))
  writeLines(
    c(paste0("共 ", length(genes), " 个 FC>1.25 上调基因；每个病人各画高低表达 vs 施旺距离："),
      genes),
    file.path(dist_dir, "本次要画的上调基因.txt")
  )
  pids_cache <- as.integer(vapply(sig_cache, function(ch) ch$patient, numeric(1)))
  names(pids_cache) <- names(sig_cache)
  pid_order <- order(pids_cache)
  for (g in genes) {
    gdir <- file.path(gene_dir, safe_gene_filename(g))
    dir.create(gdir, recursive = TRUE, showWarnings = FALSE)
    row0 <- up_tab[match(g, up_tab$gene), , drop = FALSE]
    for (nm in names(sig_cache)[pid_order]) {
      ch <- sig_cache[[nm]]
      rec <- data.frame(
        gene = g,
        patient = ch$patient,
      ...
      if (!g %in% colnames(ch$logm)) {
        rec$note <- "这个病人没有这个基因"
        idx[[paste0(g, "_", ch$patient)]] <- rec
        next
      }
      ...
      out_stub <- file.path(
        gdir,
        paste0("TNBC", ch$patient, "_", safe_gene_filename(g),
               "_high_vs_low_vs_Schwann_distance")
      )
      res <- plot_high_low_vs_distance(
        d, out_stub, g, xlab,
        plot_title = paste0("TNBC", ch$patient, "  ", g)
      )
  ...
  if ("FC" %in% names(index)) {
    index <- index[order(-index$FC, index$gene, index$patient), , drop = FALSE]
  }
  utils::write.csv(index, file.path(dist_dir, "INDEX_FC_1.25_each_gene_each_patient.csv"),
                   row.names = FALSE)
  log_msg("逐病人距离图完成：画出 ", sum(index$plotted), " / ", nrow(index),
          "（基因×病人）")
}
...
if (is.null(pb_schwann) || length(unique(si_schwann$patient)) < 2) {
  ...
  writeLines(
    "能进入综合分析的病人不足 2 例（需要近神经肿瘤 spot>=5 且 远>=10；近≤2 spot，远>4 spot）。",
    flag_no_de
  )
} else {
...
log_msg("近/远: ≤2 / >4 spot")
log_msg("综合上调: ", out_root)
log_msg("FC>1.25 每个基因每个病人距离图: ", file.path(dist_dir, "FC_1.25_each_gene"))
