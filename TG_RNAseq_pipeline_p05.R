#!/usr/bin/env Rscript
# =============================================================================
# TG BRCA 细胞 RNA-seq 分析流程（p < 0.05 平行轨道，不改 0.01 脚本）
...
# 比较 5–6 见 TG_RNAseq_TGsh_mean_vs_NTC_reps_p05.R
# 预处理：过滤低表达 + 标准化消除技术偏差
# 显著性：p < 0.05（有 padj 时 padj < 0.05）；1-vs-1 无法估计 p 时不伪造
# 子集策略：
#   A) 上调 FC >= 1 / 1.25 / 1.5 / 2
#   B) 上调排名 top 50 / 75 / 100 / 150 / 200 / 250 / 300
# 每个比较 × 每个 FC 阈值 × 每个 topN 都必须出图：
#   差异基因表/柱状图、火山图、热图、GO、通路、KEGG、GSVA、GSEA
# 结果只写 results_p05/，不要写入 results/
# =============================================================================
...
# p < 0.05 轨道（不要改成 0.01；0.01 流程在 TG_RNAseq_pipeline.R）
padj_cutoff <- 0.05
pvalue_cutoff <- 0.05
...
# -----------------------------------------------------------------------------
# 5. 差异分析
#   1-vs-1 比较：标准化后的 log 值直接相减，无 P 值（不伪造）
#   合并比较：两个 knockdown 等权平均 vs 两个 NTC 的组均值；2 vs 2 时用 limma 估计 p
# -----------------------------------------------------------------------------
pairwise_de <- function(log_mat, treat_sample, ntc_sample, comp_name) {
  ...
  log_msg(comp_name, " : ", treat_sample, " vs ", ntc_sample,
          " (1-vs-1, FC only; cannot estimate p, skip p < 0.05 filter)")
...
mean_kd_vs_ntc_de <- function(log_mat, sample_info) {
  ...
  if (n_kd >= 2 && n_ntc >= 2) {
    ...
    if (!is.null(tt) && nrow(tt) > 0) {
      ...
      log_msg("TGsh_mean_vs_NTC: limma p-values estimated (2 vs 2); filter padj < ", padj_cutoff)
    }
  } else {
    log_msg("TGsh_mean_vs_NTC: not enough replicates for p; FC/rank only")
  }
  out
}
...
analyze_one_comparison <- function(comp_name, de, full_de_for_volcano, heat_mat, sample_info, have_pvalue, gsea_de = NULL) {
  ...
  subset_dirs <- c(fc_dirs, top_dirs)
  ...
  writeLines(
    c("请打开下面两个文件夹，不要只看 GSEA：",
      ...
      "本目录是 p < 0.05 轨道，结果在 results_p05/。"),
  ...
  tryCatch({
    ...
    writeLines("全基因 GSVA，不是 FC/topN 分层结果。分层 GSVA 在 FoldChange/ 和 TopRank/ 的 GSVA/。",
...
# -----------------------------------------------------------------------------
# 10. 主流程（比较 1–4；p < 0.05；结果只写 results_p05/）
# -----------------------------------------------------------------------------
log_msg("=== p < 0.05 track: comparisons 1-4; output ", result_dir, " ===")
...
log_msg("All done (p < 0.05 track). Results in: ", result_dir)
