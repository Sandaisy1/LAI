...
#   source("TG_RNAseq_pipeline_p05.R")                 # 可选：先跑比较 1–4
#   source("TG_RNAseq_TGsh_mean_vs_NTC_reps_p05.R")    # 只加这两组
# =============================================================================

load_pipeline_functions_only <- function() {
  pipe <- "TG_RNAseq_pipeline_p05.R"
  if (!file.exists(pipe)) {
    alt <- file.path(getwd(), "TG_RNAseq_pipeline_p05.R")
    if (file.exists(alt)) pipe <- alt
  }
  if (!file.exists(pipe)) stop("找不到 TG_RNAseq_pipeline_p05.R")
  lines <- readLines(pipe, warn = FALSE)
  main_at <- grep("^# 10\\. 主流程", lines)[1]
  if (is.na(main_at) || main_at < 2) stop("无法从 p05 脚本切出函数定义")
  eval(parse(text = lines[seq_len(main_at - 1)]), envir = .GlobalEnv)
}

if (!exists("analyze_one_comparison", mode = "function") ||
    !exists("result_dir", inherits = TRUE) ||
    !grepl("results_p05", result_dir, fixed = TRUE)) {
  load_pipeline_functions_only()
}

if (!grepl("results_p05", result_dir, fixed = TRUE)) {
  result_dir <<- file.path(project_dir, "results_p05")
  log_dir <<- file.path(result_dir, "00_logs")
  dir.create(log_dir, recursive = TRUE, showWarnings = FALSE)
  log_msg("Forced output directory to results_p05/")
}
...
mean_kd_vs_one_ntc_de <- function(log_mat, sample_info, ntc_id, comp_name) {
  ...
  log_msg(comp_name, " : mean(", sh1, ", ", sh5, ") vs ", ntc,
          " (KD mean vs one NTC, FC only; cannot estimate p, skip p < 0.05 filter)")
...
prepare_extra_expression <- function() {
  if (exists("log_mat", envir = .GlobalEnv) &&
    ...
    log_msg("Reuse normalized matrix already in memory from TG_RNAseq_pipeline_p05.R")
    return(list(
  ...
  log_msg("Load and normalize expression for extra KD-mean vs single NTC comparisons (p05)")
...
log_msg("Extra p<0.05 comparisons: mean(TG_sh1, TG_sh5) vs each NTC (results_p05)")
...
for (nm in names(extra_list)) {
  ...
  tryCatch(
    ...
    error = function(e) log_msg("ERROR in extra p05 comparison ", nm, ": ", e$message)
  )
}

log_msg("Extra p05 comparisons done: ", paste(names(extra_list), collapse = ", "))
