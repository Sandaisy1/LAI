...
每个非空子集：差异基因表、火山图、热图。气泡图**先做全基因组 `enrichGO`**，再抽出 `metastasis_custom_genes.txt` 中通路的 GeneRatio、p.adjust、Count（不在自选通路上重新校正 p）。只画 **p.adjust 排名前 15**；气泡图 y 轴按 **GeneRatio 从大到小**，最大的在最上面。
