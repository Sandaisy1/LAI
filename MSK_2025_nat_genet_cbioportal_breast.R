#!/usr/bin/env Rscript
# =============================================================================
# MSK 2025 Nat Genet cBioPortal 乳腺癌：原发灶低剂量 vs 配对肺/骨转移，以及神经浸润
# 文件名：MSK_2025_nat_genet_cbioportal_breast.R
# （即“MSK_2025 nat genet_cbioportal breast”）
#
# 数据目录（脚本会自动寻找）：
#   E:/R/cBioportal breast cancer/MSK_2025 nat genet
#   或其下的 breast_msk_2025/ 解压子目录
#
# 运行：
#   setwd("E:/R/cBioportal breast cancer/MSK_2025 nat genet")
#   source("MSK_2025_nat_genet_cbioportal_breast.R")
# 也可设置环境变量 MSK2025_DIR 指向解压后的 breast_msk_2025 目录。
#
# 四个问题：
...
#
# 重要：MSK 2025（Gupta et al. Nat Genet 2025, PMID 40379787, breast_msk_2025）
# 公开包是 MSK-IMPACT 靶向测序（约 3879 例），没有全转录组 RNA。本脚本：
#   - 若目录里出现 mRNA 矩阵则优先用 RNA（与 AURORA 脚本同一套 FC 定义）
#   - 否则用离散 GISTIC 拷贝数作为剂量代理（-2/-1/0/+1/+2）
#     FC = 2^(CNA_met - CNA_primary)
#   同一患者原发+该器官转移测序对数：肺约 17，骨约 47；n < 3 不伪造 p 值。
# =============================================================================
...
dir_has_msk2025 <- function(d) {
  ...
  # 优先确认是 MSK 2025；若只有临床表也接受（用户指定了该目录）
  meta <- list.files(d, pattern = "meta_study\\.txt$", recursive = TRUE, full.names = TRUE)
  if (length(meta) > 0) {
    txt <- paste(readLines(meta[1], warn = FALSE), collapse = "\n")
    if (grepl("breast_msk_2025|MSK, Nat Genet 2025|MSK, 2025", txt, ignore.case = TRUE)) return(TRUE)
  }
  TRUE
}

resolve_msk2025_dir <- function() {
  env_dir <- Sys.getenv("MSK2025_DIR", unset = "")
  candidates <- c(
    env_dir,
    "E:/R/cBioportal breast cancer/MSK_2025 nat genet",
    "E:\\R\\cBioportal breast cancer\\MSK_2025 nat genet",
    file.path("E:/R/cBioportal breast cancer/MSK_2025 nat genet", "breast_msk_2025"),
    getwd(),
    file.path(getwd(), "breast_msk_2025"),
    file.path(getwd(), "MSK_2025 nat genet"),
    script_dir,
    file.path(script_dir, "MSK_2025 nat genet"),
    file.path(script_dir, "breast_msk_2025")
  )
  candidates <- unique(candidates[!is.na(candidates) & nzchar(candidates)])
  for (d in candidates) {
    if (dir_has_msk2025(d)) {
  ...
  stop(
    "未找到 MSK 2025 临床表 data_clinical_sample.txt。请把解压后的文件放在：",
    "E:/R/cBioportal breast cancer/MSK_2025 nat genet （或设置环境变量 MSK2025_DIR）"
  )
}

project_dir <- resolve_msk2025_dir()
result_dir  <- file.path(project_dir, "results_MSK_2025_nat_genet_cbioportal_breast")
log_dir     <- file.path(result_dir, "00_logs")
dir.create(log_dir, recursive = TRUE, showWarnings = FALSE)
log_file <- file.path(log_dir, paste0("MSK2025_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".log"))
log_msg <- function(...) {
  msg <- paste0(format(Sys.time(), "%H:%M:%S"), " | ", paste(..., collapse = ""))
  cat(msg, "\n")
  cat(msg, "\n", file = log_file, append = TRUE)
}
log_msg("MSK 2025 Nat Genet data directory: ", project_dir)
...
read_cbioportal_clinical <- function(path) {
  ...
  # 样本表以 SAMPLE_ID 开头，患者表以 PATIENT_ID 开头
...
# 优先 RNA；MSK 2025 公开包没有 RNA，回退到离散 CNA
...
if (grepl("^CNA", assay_mode)) {
  log_msg("NOTE: MSK 2025 公开包无 RNA。用 IMPACT 离散拷贝数当剂量代理。",
...
# -----------------------------------------------------------------------------
# MSK 2025 用 METASTATIC_SITE（原发多为 Not Applicable；转移为 Lung/Bone/...）
# 若解压包里仍有 SAMPLE_SITE 则优先用
...
classify_organ <- function(site, sample_type) {
  ...
  # 原发灶部位不是转移器官；Local Recurrence 不计入肺/骨配对
  is_met <- grepl("metasta", st)
  lung <- is_met & grepl("lung|lobe", s) & !grepl("liver", s)
  bone <- is_met & grepl(
    "bone|vertebra|rib|iliac|spine|femur|sternum|calvarium|perioste|humerus|tibia|scapula|sacrum|sacral|clavicle|ilium",
    s
  )
  # 不要用孤立的 "dura"：epidural 会误匹配成脑
  brain <- is_met & grepl("brain|cerebr|cerebel|occipital|\\bdura\\b|meninge|\\bcsf\\b|\\bcns\\b|leptomening", s) &
    !grepl("epidural", s)
...
pick_one_primary <- function(rows) {
  ...
  if (nrow(rows) == 1) return(rows$SAMPLE_ID[1])
  nuc <- if ("TUMOR_PURITY" %in% names(rows)) {
    suppressWarnings(as.numeric(rows$TUMOR_PURITY))
  } else if ("PERCENT_TUMOR_NUCLEI" %in% names(rows)) {
...
writeLines(
  c(
    ...
    "原发若有多枚：优先 Treatment Naive Primary；否则取肿瘤纯度（TUMOR_PURITY）最高的一枚。",
    "MSK 2025 临床表用 METASTATIC_SITE 标注转移器官；SAMPLE_TYPE=Metastasis 才计入肺/骨。",
    "Q4 的 ever_lung / ever_bone 只表示该患者另有测序的肺/骨转移灶，不是 tropism 临床旗标。",
...
export_de_set <- function(de, pairs, organ, fc_min, tag, out_root) {
  ...
  plot_volcano(
    de, hit$gene,
    paste0("MSK2025 | paired primary vs ", organ, " met | ", tag, " | ", assay_mode),
    file.path(outdir, paste0(tag, "_volcano")),
    fc_line = fc_min
  )
  plot_pair_heatmap(
    pairs, hit$gene,
    paste0("MSK2025 paired ", organ, " | ", tag, " low in primary"),
    file.path(outdir, paste0(tag, "_heatmap_paired"))
  )
  run_ora(hit$gene, de, file.path(outdir, "ORA"), paste("MSK2025", organ, tag), tag)
  hit
}
...
for (nm in names(fc_cutoffs)) {
  ...
  run_ora(only_lung, de_lung, file.path(org_dir, paste0(nm, "_lung_ONLY_ORA")),
          paste("MSK2025 lung-only", nm), paste0(nm, "_lung_ONLY"))
  run_ora(only_bone, de_bone, file.path(org_dir, paste0(nm, "_bone_ONLY_ORA")),
          paste("MSK2025 bone-only", nm), paste0(nm, "_bone_ONLY"))
}
...
if (grepl("^CNA", assay_mode)) {
  log_msg("IMPACT 面板只有约 700 基因，神经浸润基因集大部分不在芯片上；评分只用面板交集。")
}
...
# 该患者是否另有测序的肺转移灶（本队列无 tropism 旗标；不要求该原发与肺转移做成 Q1 配对）
...
pbox <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = "MSK 2025 primary neural-invasion scores vs paired lung metastasis",
    x = NULL,
...
pbox2 <- ggplot2::ggplot(
  ...
  ggplot2::theme(legend.position = "none", axis.text.x = ggplot2::element_text(angle = 20, hjust = 1)) +
  ggplot2::labs(
    title = "MSK 2025 primary neural-invasion scores vs any sequenced lung metastasis",
    x = NULL,
    y = if (grepl("^CNA", assay_mode)) "Signature score (mean CNA of gene set)" else "Signature score (mean of gene set)"
  )
