#!/usr/bin/env Rscript
# =============================================================================
# T-T6 免疫细胞亚群降维：函数库（不要单独 source 本文件）
# 这是 E:/R/flow J 的方案，比较 T6 vs T。
# 圈门、降维与 E:/R/fuction of cell 的免疫亚群分析一致，
# 但文件、目录、组别、染色表完全独立：禁止 source Flow_* / ICI_* / JY_* / JZ_*。
# 三个生物学重复不去极端值（n=3）。P1 Perforin-FITC、P2 IgD-FITC、P3 iNOS-AF488
# 都不是 His-FITC，不要套 ICI 的 His+ 母群。
# 入口：source("T_T6_Flow_dimred_pipeline.R")
# =============================================================================
...
# 读写目录：只在 E:/R/flow J；不要回退到 fuction of cell / Internation / JY / JZ
flow_primary_data_dir <- "E:/R/flow J"
flow_legacy_data_dir <- "E:/R/flow J"
flow_legacy_data_dir2 <- "E:/R/flow J"

tt6_is_foreign_dir <- function(p) {
  s <- gsub("\\", "/", as.character(p), fixed = TRUE)
  if (grepl("flow J|flowJ|flow_J|/flow-j", s, ignore.case = TRUE)) return(FALSE)
  if (grepl("cell-ljy|cell-wjz", s, ignore.case = TRUE)) return(TRUE)
  if (grepl("Internation cell immune", s, ignore.case = TRUE)) return(TRUE)
  grepl("fuction of cell|function of cell", s, ignore.case = TRUE)
}

tt6_env_dir <- function() {
...
resolve_flow_dir <- function() {
  ...
  preferred <- preferred[!vapply(preferred, tt6_is_foreign_dir, logical(1))]
  for (d in preferred) {
    if (dir.exists(d)) {
  ...
  candidates <- candidates[!vapply(candidates, tt6_is_foreign_dir, logical(1))]
  for (d in candidates) {
    if (!dir.exists(d)) next
    hits <- list.files(d, pattern = "(?i)\\.fcs$", recursive = TRUE)
    if (length(hits) > 0) {
      return(normalizePath(d, winslash = "/", mustWork = FALSE))
    }
  }
...
# -----------------------------------------------------------------------------
# 2. 文件名：T / T6
#    生物学重复 T-1/2/3 与 T6-1/2/3；文件如 T-1_P1、T6-1_P1_unmixed.fcs
#    必须先匹配 T6，再匹配 T，避免 T6-1 被拆成组别 T
# -----------------------------------------------------------------------------
flow_ctrl_group <- "T"
flow_trt_group <- "T6"
...
# 比较是 T6 vs T（处理 / 对照）。文件名里仍写 T、T6。
tt6_canon_group <- function(grp) {
  g <- toupper(gsub("[^A-Za-z0-9]", "", as.character(grp)))
  if (!nzchar(g) || is.na(g)) return(NA_character_)
  if (grepl("EVNK|NNK|EVB|ZZX|JY|JZ|ICI", g)) return(NA_character_)
  if (grepl("^H[123]?$", g) || identical(g, "EV") || grepl("^EV[123]", g)) return(NA_character_)
  if (grepl("^T6", g)) return("T6")
  if (identical(g, "T") || grepl("^T[123]([12])?$", g) || grepl("^T[123]P", g)) return("T")
  NA_character_
}

tt6_short_arm <- function(grp) {
  g <- tt6_canon_group(grp)
  if (identical(g, "T")) return("T")
  if (identical(g, "T6")) return("T6")
  NA_character_
}

# 带技术重复的名字优先，避免把 T6-1-1_P1 的 “1” 当成 panel
# 也接受没有 unmixed/raw 后缀的 T-1_P1.fcs
flow_re_tech <- "^(?:TT6[_-]?)?(T6|T)[-_ ]?([123])[-_ ]([12])[-_ ]+(?:PANEL[-_ ]?)?P?0?([123])(?:[-_ ].*)?\\.fcs$"
flow_re_bio <- "^(?:TT6[_-]?)?(T6|T)[-_ ]?([123])[-_ ]+(?:PANEL[-_ ]?)?P?0?([123])(?:[-_ ].*)?\\.fcs$"
flow_re_folder_tech <- "^([123])[-_ ]([12])[-_ ]+(?:PANEL[-_ ]?)?P?0?([123])(?:[-_ ].*)?\\.fcs$"
...
parse_fcs_filename <- function(path) {
  ...
  if (length(hit) >= 5) {
    ...
    tech <- hit[4]
    panel_n <- hit[5]
    kind <- if (grepl("unmixed", b, ignore.case = TRUE)) "unmixed" else if (grepl("raw", b, ignore.case = TRUE)) "raw" else "fcs"
  } else {
  ...
  grp <- tt6_canon_group(grp)
  if (is.na(grp) || !nzchar(grp)) return(NULL)
  arm <- tt6_short_arm(grp)
...
list_unmixed_files <- function(root) {
  ...
  if (any(!ok) && length(files) > 0) {
    skipped <- basename(files[!ok])
    if (length(skipped) > 8L) skipped <- c(skipped[seq_len(8L)], "...")
    log_msg("Skip unmatched FCS names: ", paste(skipped, collapse = ", "))
  }
...
# 若有技术重复，先在生物学重复内平均；T-T6 不去极端生物学重复，统计 n=3。
# 不要把 T-1 与 T6-1 当成同一组。不要套用 Flow/JY/JZ 的 n=2。
...
# T-T6 与 ICI 一样保留全部生物学重复（n=3）；不要改成 Flow/JY/JZ 的去极端值。
...
flow_should_trim_bio <- function(trim_bio = NULL) {
  ...
  isTRUE(get0("flow_trim_bio_extremes", ifnotfound = FALSE, inherits = TRUE))
}
...
# -----------------------------------------------------------------------------
# 3. Panel 地图
# Windows 常把“另存为”弄成 T_T6_flow_panel_map.json.txt（资源管理器显示“文本文档”）
# -----------------------------------------------------------------------------
panel_map_search_dirs <- function() {
  ...
  dirs <- dirs[!vapply(dirs, tt6_is_foreign_dir, logical(1))]
...
find_panel_map_file <- function(dirs = panel_map_search_dirs()) {
  ...
  names <- c(
    ...
    "T_T6_flow_panel_map.json.txt",
    "T_T6_flow_panel_map.txt"
  )
  ...
  fuzzy <- unlist(lapply(dirs, function(d) {
    list.files(d, pattern = "^T_T6_flow_panel_map", ignore.case = TRUE, full.names = TRUE)
  }), use.names = FALSE)
...
load_panel_map <- function() {
  ...
  stop(
    "找不到 T_T6_flow_panel_map.json。\n",
    "Windows 若类型是“文本文档”、图标是记事本，真实文件名多半是 T_T6_flow_panel_map.json.txt。\n",
    "在资源管理器：查看 → 去掉“隐藏已知文件类型的扩展名”，再改名为 T_T6_flow_panel_map.json。\n",
    "已搜索：\n", listing
  )
}
...
export_major_subset_dimred <- function(cells, panel_id, out_dir) {
  ...
  for (mj in majors) {
    ...
    tryCatch({
      ...
      ttl <- paste0(panel_id, "  ", lab, "  subsets  T | T6")
      ...
      save_split_dr(
        ...
        file.path(class_dir, paste0(tag, "_tSNE_subset_T6_vs_T")),
        n_keys
      )
      save_split_dr(
        plot_split_lineage(sub, "UMAP1", "UMAP2", panel_id, "UMAP-1", "UMAP-2",
                           ttl, color_mode = "subset"),
        file.path(class_dir, paste0(tag, "_UMAP_subset_T6_vs_T")),
        n_keys
      )
...
export_subset_gate_figures <- function(cells, panel_id, out_dir) {
        ...
        if (is.null(samp) || nrow(samp) < 2) {
          ...
          if (is.null(samp_bio) || nrow(samp_bio) < 2) {
            ...
            if (nrow(d_ctrl) < 8 || nrow(d_trt) < 8) {
              ...
              stub <- paste0(panel_id, "_", gsub("[^A-Za-z0-9]+", "_", spec$lineage), "_T6_vs_T")
  ...
  if (length(stat_rows)) {
    ...
    utils::write.csv(st, file.path(sub_dir, paste0(panel_id, "_subset_T6_vs_T_stats.csv")), row.names = FALSE)
  }
...
export_p1_activation_stats <- function(cells, out_dir) {
  ...
  utils::write.csv(stats, file.path(out_dir, "P1_TNK_activation_T6_vs_T_stats.csv"), row.names = FALSE)
...
export_p2_activation_stats <- function(cells, out_dir) {
  ...
  stats_pct$metric <- "pct_positive"
  stats <- rbind(stats_mfi, stats_pct)
  utils::write.csv(stats, file.path(out_dir, "P2_Bcell_activation_T6_vs_T_stats.csv"), row.names = FALSE)
...
export_p3_activation_stats <- function(cells, out_dir) {
  ...
  stats_pct$metric <- "pct_positive"
  stats <- rbind(stats_mfi, stats_pct)
  utils::write.csv(stats, file.path(out_dir, "P3_APC_activation_T6_vs_T_stats.csv"), row.names = FALSE)
...
functional_state_specs <- function(panel_id) {
  ...
  if (identical(panel_id, "P1")) {
    act <- c("CD69", "IFN-g", "TNF-a", "GZMB", "Perforin")
...
export_dimred_plots <- function(cells, med, annot, freq_df, panel_id, out_dir, umap_is_pca = FALSE, tsne_is_pca = FALSE) {
  ...
  split_ttl <- paste0(panel_id, "  T | T6")
  log_msg(
    panel_id,
  ...
  save_split_dr(p_major_tsne, file.path(out_dir, paste0(panel_id, "_T6_vs_T_tSNE_major_split")), n_major)
  save_split_dr(p_major_umap, file.path(out_dir, paste0(panel_id, "_T6_vs_T_UMAP_major_split")), n_major)
  ...
  save_split_dr(p_subset_tsne, file.path(out_dir, paste0(panel_id, "_T6_vs_T_tSNE_lineage_split")), n_subset)
  save_split_dr(p_subset_umap, file.path(out_dir, paste0(panel_id, "_T6_vs_T_UMAP_lineage_split")), n_subset)
  save_split_dr(p_subset_umap, file.path(out_dir, paste0(panel_id, "_T6_vs_T_UMAP_lineage_split_joint")), n_subset)
  ...
  save_gg(plot_density_split(cells, "UMAP1", "UMAP2", paste(tag, "-", umap_lab, "density T6 vs T")),
          file.path(out_dir, paste0(panel_id, "_UMAP_density_T6_vs_T")), width = 10, height = 5)
  ...
  save_gg(plot_density_split(cells, "tSNE1", "tSNE2", paste(tag, "-", tsne_lab, "density T6 vs T")),
          file.path(out_dir, paste0(panel_id, "_tSNE_density_T6_vs_T")), width = 10, height = 5)
...
analyze_one_panel <- function(panel_id, file_tab, use_demo) {
  ...
  if (flow_should_trim_bio()) {
    log_msg(panel_id, " T6 vs T stats: tech reps averaged, then drop 1 extreme bio-rep (max or min) per group; n=2")
  } else {
    log_msg(panel_id, " T6 vs T stats: tech reps averaged to n=3 bio (no extreme dropped)")
  }
...
# -----------------------------------------------------------------------------
# 10. 本文件只提供函数。分析入口是 T_T6_Flow_dimred_pipeline.R
# -----------------------------------------------------------------------------
tt6_engine_loaded <- TRUE
log_msg("T_T6_flow_engine.R: function library only; run T_T6_Flow_dimred_pipeline.R")
