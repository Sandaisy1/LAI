...
load_tt6_engine_functions <- function() {
  if (isTRUE(get0("tt6_engine_loaded", ifnotfound = FALSE))) {
...
export_one_major_trajectory <- function(cells, panel_id, major, out_dir) {
  ...
  save_split_dr(plot_trajectory_tree(sub, fit, panel_id, ttl, facet_group = FALSE),
                ...
                n_keys, facet = FALSE)
  save_split_dr(plot_trajectory_tree(sub, fit, panel_id, paste0(ttl, "  T | T6"), facet_group = TRUE),
                file.path(out_dir, paste0(tag, "_trajectory_T6_vs_T")),
                n_keys, facet = TRUE)
  if (any(is.finite(sub$pseudotime))) {
    save_gg(plot_pseudotime_group(sub, paste0(panel_id, "  ", major, "  pseudotime T6 vs T")),
            file.path(out_dir, paste0(tag, "_pseudotime_T6_vs_T")),
...
export_panel_major_trajectory <- function(cells, panel_id, out_dir) {
  ...
  save_split_dr(
    plot_trajectory_tree(sub, fit, panel_id, paste0(ttl, "  T | T6"), facet_group = TRUE),
    file.path(out_dir, paste0(panel_id, "_major_trajectory_T6_vs_T")),
    n_keys, facet = TRUE
  )
